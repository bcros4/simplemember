...
class SwpmForm {
 ...
 protected function staff_code() {
        $staff_code = filter_input(INPUT_POST, 'staff_code', FILTER_UNSAFE_RAW);
        if (empty($staff_code)) {return;}
        $saned = wp_kses($staff_code, array());
        $this->sanitized['staff_code'] = $saned;        
        return;
    }
protected function height() {
        $height = filter_input(INPUT_POST, 'height', FILTER_UNSAFE_RAW);
	if (empty($height)) {return;}
        $saned = wp_kses($height, array());
        $this->sanitized['height'] = $saned;        
        return;
    }
protected function weight() {
        $weight = filter_input(INPUT_POST, 'weight', FILTER_UNSAFE_RAW);
        if (empty($weight)) {return;}
        $saned = wp_kses($weight, array());
        $this->sanitized['weight'] = $saned;        
        return;
    }
